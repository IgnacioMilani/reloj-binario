...
int main(int argc,char *argv[]){
	...
	int sec=tlocal->tm_sec;
	for(i=0;i<=15;i++){				//Prueba de 15 segundos
		rc= serialport_writebyte(fd, (uint8_t)sec);		//Envia el numero
		if(rc==-1) perror("Error de escritura");		//Variable de chequeo
		sleep(1);
		if (sec<59)  sec++;			//Si sec=58 pasa a sec=59
		else sec=0;				//Si sec=59 pasa a sec=0
	}
